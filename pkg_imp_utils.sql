...
as
  ...
  function create_def_import(
    ...
    as
        v_last_run_rid imp_run.imp_run_id%type := 0;
        v_imp_spec_id imp_spec.imp_spec_id%type;
        v_pid imp_spec.program_id%type;
    begin
        -- 0) Find last run of this import
        select imp_spec_id, program_id into v_imp_spec_id, v_pid
        from imp_run where imp_run_id = p_rid;
        ...
        begin
            ...
            --Recovered,
            --Interrupted
            select imp_run_id into v_last_run_rid from (
                select ir.imp_run_id
                from imp_run ir join process p on (p.process_id = ir.process_id)
                where ir.imp_spec_id = v_imp_spec_id and ir.program_id = v_pid
                and ir.imp_run_id <> p_rid and p.status_id in (6, 7, 12, 14)
        ...
        from
          ...
          join imp_run_grid_incr nirg on (nirg.imp_run_id = p_rid and nirg.row_num = 0 and nirg.data = c.name)
          join imp_run_grid_incr oirg on (oirg.imp_run_id = v_last_run_rid and oirg.row_num = 0 and oirg.data = c.name)
        where
          spec.imp_spec_id = v_imp_spec_id;
        ...
        from (
          ...
          where imp_run_id = p_rid and row_num > 0 and col_num in (select new_col from imp_run_pk_col)
          group by imp_run_id, row_num
        ) nirg
        left outer join (
          select row_num, listagg(data, ',' on overflow truncate without count) within group (order by col_num) pk
          from imp_run_grid_incr
          where imp_run_id = v_last_run_rid and row_num > 0 and col_num in (select old_col from imp_run_pk_col)
          group by imp_run_id, row_num
        ...
        on (oirg.imp_run_id = v_last_run_rid and oirg.row_num = 0 and oirg.data = nirg.data)
        where nirg.imp_run_id = p_rid and nirg.row_num = 0;
        ...
        from imp_run_grid_incr
        where imp_run_id = p_rid and col_num not in (select new_col from imp_run_pk_col) and row_num > 0
        minus
    ...
    function get_col_num(
        ...
        return imp_run_grid_incr.col_num%type as

        v_colnum number;
    begin
        select col_num into v_colnum from imp_run_grid
        where imp_run_id = p_rid and row_num = 0 and upper(data) = upper(p_colname);
         ...
         where imp_run_id = p_rid and data is not null
           ...
           and col_num = (
                select col_num 
                  from imp_run_grid
                 where imp_run_id = p_rid 
                   and row_num = 0 and upper(data) = upper(p_colname));
